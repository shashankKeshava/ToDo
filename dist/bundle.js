...
/***/ (function(module, exports, __webpack_require__) {
	...
	function render(el, state) {
	    var todoItems = state.todos.map(function (toDo) {
	        ...
	        } else if (state.filter == 'open' && !toDo.done) {
	            return renderTodoItem(toDo);
	        } else if (state.filter == 'closed' && toDo.done) {
	            return renderTodoItem(toDo);
	        } else return "";
	    }).join('');
	    el.innerHTML = renderApp(renderInput(), renderTodos(todoItems), filter(state));
	}
...
/***/ (function(module, exports, __webpack_require__) {
	...
	function registerEventHandlers() {
	    ...
	    (0, _events.listen)('click', '.todo_filter_options', function (event) {
	        var filter = event.target.id;
	        _state.todos.dispatch((0, _actions.filterToDo)(filter));
	    });
	}
